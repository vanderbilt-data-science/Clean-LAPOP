...
```{r}
country <- read_dta("1326054827Argentina LAPOP merge 2008-2014 v3.0_W_dta.zip")
  factor_country <- as_factor(country)
  x <- 1
  while(x < length(factor_country)){
    colnames(factor_country)[x] <- paste(colnames(factor_country)[x], "factor", sep = "-")
    x <- x + 1
  }
  factor_country
```

NEW for creating factored and unfactored columns together so that they can be exchanged
```{r}
factor_country <- function(country){
...
(colnames(arg_test[1]))
paste(colnames(arg_test[1]), "x")

colnames(arg_test)[1] <- paste(colnames(arg_test[1]), "factor", sep = "-")

q_list <- questions(arg_test)
q_list

x <- attributes(arg_test)$names[1]
attributes(arg_test[[x]])$label

attributes(arg_test$pais-factor)$label <- 'new'


labels(arg_test, which = q_list[1]) <- paste(q_list[1], "factor", sep = "-")
```
