...
arg_question_test <- semi_join(argentina_clean, common_questions, by = "question")

#Function for joining dataframes to ensure each country has only the common questions
total_common_questions <- function(country, question_dfr) {
  country %>% 
    semi_join(question_dfr, by = "question")
}

#Test to make sure the function works
bol_question_test <- total_common_questions(bolivia_clean, common_questions)

countries_common_only <- reduce2(countries, common_questions, total_common_questions)
```
