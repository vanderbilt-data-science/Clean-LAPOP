...
These variables are not to be shown in the visualizations, so we made a list of them to utilize in the next function.
If you want more variables to not be shown, just add them to the list.
...
```
#Final clean
This function uses the list made above and removes the column names that are in the list variables. It also gets the country ready to be combined with the other countries fully.
```{r}
clean <- function(country){
  #filters the varaibles
  country <- country %>%
   filter(!(column_name %in% variables)) %>%
    #selects the variables that are no longer needed
...
In order to write a .csv file of the data, we combined all the countries using `bind_rows()` and saved it in a data frame called complete. However, because Tableau can only handle 15 million rows, we subset the data by filtering out for when wave is 2006, 2008, 2010, and 2012 and used this data frame to write the .csv.
...
complete <- complete %>% filter(!(pais == 41 & wave > 2008))
complete <- complete %>% filter(!(pais == 40 & wave > 2008))
complete <- complete %>% filter(!(pais == 16 & wave > 2008))
complete <- complete %>% filter(!(pais == 13 & wave > 2008))
complete <- complete %>% filter(!(pais == 17 & wave > 2008))
complete <- complete %>% filter(!(pais == 10 & wave > 2008))
complete <- complete %>% filter(!(pais == 27 & wave > 2008 & wave < 2014))
complete <- complete %>% filter(!(pais == 35 & wave > 2008 & wave < 2014))
complete <- complete %>% filter(!(pais == 9 & wave < 2014))
