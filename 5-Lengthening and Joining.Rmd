...
# Structure
In this section we lengthen the datasets so that there are only 5 columns: pais, wave, uniq_id, column_name, and answer. We also read in the categories from a csv that contains all the categories and questions. There will be combined to the lengthened datasets at the end.

There are four versions of the lengthen function. The first two are the functions that should be used for the unfactored versions of the merged and 2016/17 countries. The last two are for the factored versions.

When we apply the lengthen function, we also use `left_join()` to add a column for questions and categories.

The process is repeated twice for each country - once for the unfactored versions and then again for the factored versions.

```{r}
factored_categories <- read_csv('/Users/lindsayhardy/Clean-LAPOP/Lapop Categories Factored.csv')
factored_categories <- factored_categories %>% select(column_name_factor, category)
factored_categories %>% assert(is_uniq, column_name_factor)
...
unfactored_questions <- read_csv('/Users/lindsayhardy/Clean-LAPOP/updated unfactor questions.csv')
unfactored_questions <- unfactored_questions %>% select(column_name, question_unfactor)
factored_questions <- read_csv('/Users/lindsayhardy/Clean-LAPOP/updated factor questions.csv')
factored_questions <- factored_questions %>% select(column_name_factor, question_factor)
```

```{r}
tidy_spread <- function(x){
  x <- x  %>% select(-column_name) %>%
    spread(question_unfactor, answer)
}
```

```{r}
tidy_spread_factor <- function(x){
  x <- x %>% select(-column_name_factor) %>%
    spread(question_factor, answer_factor)
}
```
...
argentina_joined <- argentina_joined %>%
  left_join(unfactored_questions, by = 'column_name')
argentina_joined <- argentina_joined %>% filter(is.na(category) == FALSE)
...
brazil_joined <- brazil_joined %>%
  left_join(unfactored_questions, by = 'column_name')
brazil_joined <- brazil_joined %>% filter(is.na(category) == FALSE)
...
chile_joined <- chile_joined %>%
  left_join(unfactored_questions, by = 'column_name')
chile_joined <- chile_joined %>% filter(is.na(category) == FALSE)
...
panama_joined <- panama_joined %>%
  left_join(unfactored_questions, by = 'column_name')
panama_joined <- panama_joined %>% filter(is.na(category) == FALSE)
...
paraguay_joined <- paraguay_joined %>%
  left_join(unfactored_questions, by = 'column_name')
paraguay_joined  <- paraguay_joined  %>% filter(is.na(category) == FALSE)
...
peru_joined <- peru_joined %>%
  left_join(unfactored_questions, by = 'column_name')
peru_joined  <- peru_joined  %>% filter(is.na(category) == FALSE)
...
uruguay_joined <- uruguay_joined %>%
  left_join(unfactored_questions, by = 'column_name')
uruguay_joined  <- uruguay_joined %>% filter(is.na(category) == FALSE)
...
venezuela_joined <- venezuela_joined %>%
  left_join(unfactored_questions, by = 'column_name')
venezuela_joined  <- venezuela_joined %>% filter(is.na(category) == FALSE)
...
bolivia_joined <- bolivia_joined %>%
  left_join(unfactored_questions, by = 'column_name')
bolivia_joined  <- bolivia_joined %>% filter(is.na(category) == FALSE)
...
canada_joined <- canada_joined %>%
  left_join(unfactored_questions, by = 'column_name')
canada_joined  <- canada_joined %>% filter(is.na(category) == FALSE)
...
colombia_joined <- colombia_joined %>%
  left_join(unfactored_questions, by = 'column_name')
colombia_joined  <- colombia_joined %>% filter(is.na(category) == FALSE)
...
costa_joined <- costa_joined %>%
  left_join(unfactored_questions, by = 'column_name')
costa_joined  <- costa_joined %>% filter(is.na(category) == FALSE)
...
ecuador_joined <- ecuador_joined %>%
  left_join(unfactored_questions, by = 'column_name')
ecuador_joined <- ecuador_joined %>% filter(is.na(category) == FALSE)
...
elsalvador_joined <- elsalvador_joined %>%
  left_join(unfactored_questions, by = 'column_name')
elsalvador_joined <- elsalvador_joined %>% filter(is.na(category) == FALSE)
...
guatemala_joined <- guatemala_joined %>%
  left_join(unfactored_questions, by = 'column_name')
guatemala_joined <- guatemala_joined %>% filter(is.na(category) == FALSE)
...
honduras_joined <- honduras_joined %>%
  left_join(unfactored_questions, by = 'column_name')
honduras_joined <- honduras_joined %>% filter(is.na(category) == FALSE)
...
mexico_joined <- mexico_joined %>%
  left_join(unfactored_questions, by = 'column_name')
mexico_joined <- mexico_joined %>% filter(is.na(category) == FALSE)
...
nicaragua_joined <- nicaragua_joined %>%
  left_join(unfactored_questions, by = 'column_name')
nicaragua_joined <- nicaragua_joined %>% filter(is.na(category) == FALSE)
...
usa_joined <- usa_joined %>%
  left_join(unfactored_questions, by = 'column_name')
usa_joined <- usa_joined %>% filter(is.na(category) == FALSE)
...
lengthen_factor <- function(country) {
  ...
  country <- country %>%
    #These are excluded because they are repeated
    #Brings together the following columns
    gather(column_name_factor, answer_factor, estratopri_factor:formatq_factor)
  #Takes length of column_name and arguments from long_questions
  country <- country %>% 
    select(pais_factor, wave_factor, uniq_id_factor, column_name_factor, answer_factor)
  return(country)
}
```
...
belize_category_factor <- belize_category_factor %>% filter(is.na(category) == FALSE)
...
argentina_joined_factor <- argentina_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
argentina_joined_factor <- argentina_joined_factor %>% filter(is.na(category) == FALSE)
...
brazil_joined_factor <- brazil_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
brazil_joined_factor <- brazil_joined_factor %>% filter(is.na(category) == FALSE)
...
chile_joined_factor <- chile_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
chile_joined_factor <- chile_joined_factor %>% filter(is.na(category) == FALSE)
...
panama_joined_factor <- panama_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
panama_joined_factor <- panama_joined_factor %>% filter(is.na(category) == FALSE)
...
paraguay_joined_factor <- paraguay_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
paraguay_joined_factor <- paraguay_joined_factor %>% filter(is.na(category) == FALSE)
...
peru_joined_factor <- peru_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
peru_joined_factor <- peru_joined_factor %>% filter(is.na(category) == FALSE)
...
uruguay_joined_factor <- uruguay_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
uruguay_joined_factor <- uruguay_joined_factor %>% filter(is.na(category) == FALSE)
...
venezuela_joined_factor <- venezuela_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
venezuela_joined_factor <- venezuela_joined_factor %>% filter(is.na(category) == FALSE)
...
bolivia_joined_factor <- bolivia_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
bolivia_joined_factor <- bolivia_joined_factor %>% filter(is.na(category) == FALSE)
...
canada_joined_factor <- canada_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
canada_joined_factor <- canada_joined_factor %>% filter(is.na(category) == FALSE)
...
colombia_joined_factor <- colombia_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
colombia_joined_factor <- colombia_joined_factor %>% filter(is.na(category) == FALSE)
...
costa_joined_factor <- costa_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
costa_joined_factor <- costa_joined_factor %>% filter(is.na(category) == FALSE)
...
ecuador_joined_factor <- ecuador_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
ecuador_joined_factor <- ecuador_joined_factor %>% filter(is.na(category) == FALSE)
...
elsalvador_joined_factor <- elsalvador_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
elsalvador_joined_factor <- elsalvador_joined_factor %>% filter(is.na(category) == FALSE)
...
guatemala_joined_factor <- guatemala_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
guatemala_joined_factor <- guatemala_joined_factor %>% filter(is.na(category) == FALSE)
...
honduras_joined_factor <- honduras_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
honduras_joined_factor <- honduras_joined_factor %>% filter(is.na(category) == FALSE)
...
mexico_joined_factor <- mexico_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
mexico_joined_factor <- mexico_joined_factor %>% filter(is.na(category) == FALSE)
...
nicaragua_joined_factor <- nicaragua_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
nicaragua_joined_factor <- nicaragua_joined_factor %>% filter(is.na(category) == FALSE)
...
```{r}
unfactor <- argentina_total %>% select(pais:question_unfactor)
arg_un <- tidy_spread(unfactor)
factor <- argentina_total %>% select(pais_factor:question_factor)
arg_f <- tidy_spread_factor(factor)
```