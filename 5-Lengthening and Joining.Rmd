...
## Reading in categories from CSV
```{r}
#Factored categories
...
factored_categories %>% 
  assert(is_uniq, column_name_factor)

#Unfactored categories
...
unfactored_categories <- unfactored_categories %>% 
  elect(column_name, category)
unfactored_categories %>%
  assert(is_uniq, column_name)
```

##Reading in questions from CSV
```{r}
#Factored questions
...
## Functions to lengthen unfactored datasets
### Use for merged files
```{r}
...
## Applying lengthening and joining merged datasets with 16/17 datsets
```{r}
#Belize
...
#Argentina
#Lengthens unfactored dataset that has unique ID
argentina_long_unfactor <- lengthen(argentina_unique_unfactor)
#Lengthens 2016 unfactored datset
...
argentina_joined <- union_all(argentina_long_unfactor, argentina16_long_unfactor) %>% 
  #Changes answer column to numeric
...
### Use for factored 2016/17 files
```{r}
...
## Applying lengthening and joining factor merged datasets with factor 16/17 datsets
```{r}
#Belize
...
belize_category_factor <- belize_category_factor %>% left_join(factored_categories, by = 'column_name_factor')
...
argentina_joined_factor <- argentina_joined_factor %>%
  left_join(factored_questions, by = 'column_name_factor')
argentina_total <- argentina_joined_factor %>% left_join(argentina_joined, by = 'category')

t <- argentina_joined_factor %>% select(-category)
test <- bind_cols(argentina_joined,t)


#Brazil
brazil_long_factor <- lengthen_factor(brazil_unique_factor)
brazil16_long_factor <- lengthen_16factor(brazil_16_factor)
brazil_joined_factor <- union_all(brazil_long_factor, argentina16_long_factor) 
...
guatemala_joined_factor<- guatemala_long_factor %>% 
  left_join(factored_categories, by = 'column_name_factor')
